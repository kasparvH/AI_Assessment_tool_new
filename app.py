...
def get_active_questions():
    """Returns the question DataFrame, sliced to 4 per dimension when test mode is on."""
    df = get_questions()
    test_on = st.session_state.get("_test_mode_override", False) or is_test_mode()
    if test_on:
...
def screen_welcome():
    ...
    test_mode_on = st.sidebar.toggle(
        "Enable test mode (4 questions per dimension)",
        value=st.session_state.get("_test_mode_override", False),
        key="test_mode_toggle",
    )
    st.session_state["_test_mode_override"] = test_mode_on
    if test_mode_on:
        st.sidebar.caption("Test mode active — 4 questions per dimension (28 total instead of 265).")
    st.sidebar.markdown("---")
    ...
    if st.session_state.get("_test_mode_override", False) or is_test_mode():
        n = get_test_questions_per_dim()
        st.warning(
            f"🧪 **Test mode active** — this session contains {n} questions per dimension "
            f"({7 * n} questions total instead of 265)."
        )
...
def screen_assessment():
    ...
    test_mode = st.session_state.get("_test_mode_override", False) or is_test_mode()
