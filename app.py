...
def render_admin_portal():
    ...
    tab_sessions, tab_reports, tab_new, tab_settings = st.tabs([
        "📋 All Sessions",
        "📄 Reports & Downloads",
    ...
    # ── Tab 4: Settings ──
    with tab_settings:
        from config_manager import set_test_mode

        st.markdown("### 🧪 Test Mode")
        st.markdown(
            "When test mode is active, only **4 questions per dimension** are shown "
            "(28 total instead of 265). Use this for testing and demos."
        )

        current = is_test_mode()
        new_val = st.toggle(
            "Enable test mode",
            value=current,
            key="admin_test_mode_toggle",
        )

        if new_val != current:
            set_test_mode(new_val)
            if new_val:
                st.success("✅ Test mode enabled — 4 questions per dimension (28 total).")
            else:
                st.success("✅ Test mode disabled — all 265 questions active.")
            st.rerun()

        if current:
            st.info("🧪 Test mode is currently **ON** — 4 questions per dimension.")
        else:
            st.info("✅ Test mode is currently **OFF** — all 265 questions active.")
